---
version: alpha
name: DealDuniya-design
description: |
  DealDuniya is an image-first product discovery publication. Principles taken from the Pinterest design reference: photography leads, chrome stays quiet, grids are for browsing, cards are 16px, gutters stay tight, and one clear CTA colour is enough. Do not copy Pinterest Red, Pin Sans, the Pinterest logo, or Pinterest layouts. DealDuniya uses cream paper, gold from the existing logo (#a8842b light / #d4af37 dark), Instrument Serif for display, and Manrope for UI. Dark mode is a warm charcoal, not a simple invert. No fake ratings, no urgency badges, no grain overlay.
